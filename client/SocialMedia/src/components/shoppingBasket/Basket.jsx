        ...
        <aside className="cartBasketContainer">
                    ...
                    <div key={item._id} className='row'>
                        ...
                        item.name.slice(0,13) + '...'
                            ...
                            <div className="itemCount">
                                {item.quantity} {item.quantity === 1 || item.quantity === 0 ? 'item' : 'items'} x R {item.item.price}
                            </div>
                        <div className='totalItemWrapper'>
                            <div className="itemsTotal">Total:</div>
                            <div style={{display: 'flex', alignItems: 'end'}}>
                                <div className='itemsTotal rand'>R</div>
                                <div style={{fontWeight: 'bold', fontSize: "13px"}}>{item.quantity * item.item.price}</div>
                            </div>
                        </div>
                    </div>
                ))}
            ...
            {cartItems.length !== 0 && (
                ...
                <div className='col'>
                    ...
                    <div><strong>{cartItems.length === 0 ? "No Items": cartItems.length === 1 ? cartItems.length + ' item' : cartItems.length + ' items'}</strong></div> 
                </div>
                </>
            )}
        </aside>
